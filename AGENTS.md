...
- `src/ui/` — Obsidian modals: unified card edit modal, overlay/spread/widget/calendar editors, symbol & FRED & macro & remote-quote (tx/em) search modals, folder suggester, stepper, `cleanup-modal.ts` (shared checklist + two-step confirm for the cleanup tools).
