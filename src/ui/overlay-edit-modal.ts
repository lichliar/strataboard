...
import type { ChartTheme, CustomSourceDef, OverlaySpec, SeriesPeriod, SeriesRef } from "../types";
...
export class OverlayEditModal extends Modal {
  ...
  private renderSeriesPage(pageEl: HTMLElement) {
    pageEl.createDiv({
      ...
      text: t("至少一个系列；行情类系列按区间首个数据点归一化为涨跌幅（%）。"),
    });
    this.rowsEl = pageEl.createDiv({ cls: "fc-calc-series-rows" });
    for (const ref of this.initialSeries) {
      this.addRow(ref);
    }
    const addBtn = pageEl.createEl("button", { cls: "fc-add-row", text: t("+ 添加系列"), attr: { type: "button" } });
  ...
  private renderDisplayPage(pageEl: HTMLElement) {
    new Setting(pageEl)
      .setName(t("标准化"))
      .setDesc(t("以首条系列起点为 100% 基准，将行情类序列折算为涨跌幅（%）。"))
      .addToggle((toggle) =>
